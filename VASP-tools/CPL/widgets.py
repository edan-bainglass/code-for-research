...
class PremiumCheckButtons(CheckButtons, AxesWidget):
    def __init__(self, ax, frags, actives, linecolor="k", showedge=True, **kw):
        AxesWidget.__init__(self, ax)
        ...
        labels = [re.sub('([0-9]+)', r'$_{\1}$', f.name) for f in frags]
        self.box = ax.legend(handles=[object() for f in frags],
                             labels=labels,
