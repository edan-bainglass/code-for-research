...
# CONVERGENCE TESTING ###############################################################################################
  ...
  if [[ $tType == K ]]; then k1=$(($(getKps | getNumber | sort -nr | head -1) + kInc)); kPoints $k1 $even; fi
  for i in $(seq $cycles); do
    ...
    case $tType in
      K) k1=$((k1 + kInc)); kPoints $k1 $even;;
      E) val=$((val + 25)); setTag ENCUT $val ;;
      N) nBands=$((nBands + step)); setTag NBANDS $nBands ;;
      S) backUp C $ismear $(echo $sig | sed 's/^\./0\./') $(getKps | sed 's/ /-/g')
         sig=$(echo $sig + $step | bc); setTag SIGMA $(echo $sig | sed 's/^\./0\./') ;;
    esac
  done
